function compareSourceNode(){

var components = new Array();
var x1,x2,y1,y2,text,dist;
	...
	if(normalArray.length>0){
		for(var i = 0;i < normalArray.length; i++){
			if(sendersArray[0][2] - normalArray[i][2] < 150 && sendersArray[0][2] - normalArray[i][2] > -150 && sendersArray[0][3] - normalArray[i][3] < 150 && sendersArray[0][3] - normalArray[i][3] > -150){
				...
				components = ["S1",normalArray[i][0],normalArray[i][0],dist,1,x1,y1,x2,y2];
				nodeTableArray.push(components);
				createNodeTable("S1",normalArray[i][0],normalArray[i][0],dist,1,x1,y1,x2,y2);
				//components = [normalArray[i][0],"S1","S1",dist,1];
				//nodeTableArray.push(components);
				//createNodeTable(normalArray[i][0],"S1","S1",dist,1);
				addTableArray(normalArray[i][0],"S1");
				components = ("S1");
	...
	if(receiversArray.length>0){
		...
		if(sendersArray[0][2] - receiversArray[0][2] < 150 && sendersArray[0][2] - receiversArray[0][2] > -150 && sendersArray[0][3] - receiversArray[0][3] < 150 && sendersArray[0][3] - receiversArray[0][3] > -150){
			if (receiversArray[0][2] == sendersArray[0][2]){	
				...
				components = ["S1","R1","R1",dist,1,x1,y1,x2,y2];
				nodeTableArray.push(components);
				createNodeTable("S1","R1","R1",dist,1,x1,y1,x2,y2);
				//components = ["R1","S1","S1",dist,1];
				//nodeTableArray.push(components);
				//createNodeTable("R1","S1","S1",dist,1);
				addTableArray("S1","R1");
				components = ("S1");
				nodeAvailable.push(components);		
